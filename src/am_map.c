...
#define AUTOPAGE_WIDTH	320
#define AUTOPAGE_HEIGHT	158
...
//			dmapx=0;  //goes into the automap.
...
#else
	{
		...
		for (y=0;y<finit_height;y++) {
			byte *autopage_line = maplump;
			byte *dest_line = dest;
			int width, x;
			
			autopage_line += ((y+mapystart)%AUTOPAGE_HEIGHT)*AUTOPAGE_WIDTH;
			width = (AUTOPAGE_WIDTH-(mapxstart%AUTOPAGE_WIDTH));
			x = 0;
			while (x<finit_width) {
				memcpy(dest_line, autopage_line, width);
				dest_line += width;
				x += width;
				width = AUTOPAGE_WIDTH;
				if (x+width>finit_width) {
					width = finit_width-x;
				}
			}
		...
		default:
  		{
  				...
  				#define DOT(xx,yy,cc) fb[(yy)*sysvideo.pitch+(xx)]=(cc) //the MACRO!
...
//	}
	...
	{
		oldyy = yy;
		oldyyshifted = yy*sysvideo.pitch;
	}
...
void AM_drawCrosshair(int color)
{
	...
	fb1 += ((f_h+1)>>1)*sysvideo.pitch;
	fb1 += f_w>>1;

	*fb1 = color; // single point for now
}
...
void AM_Drawer(void)
{
	...
	f_w = finit_width = sysvideo.width;
	f_h = finit_height = sysvideo.height-((SBARHEIGHT*sysvideo.height)/SCREENHEIGHT);
