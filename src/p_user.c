...
#define MIN_LOOKDIR	((-110*sysvideo.height)/SCREENHEIGHT)
#define MAX_LOOKDIR	((90*sysvideo.height)/SCREENHEIGHT)
...
void P_MovePlayer(player_t *player)
{
	...
	if(look)
	{
		...
		else
		{
			...
			cur_lookdir = player->lookdir;
			player->lookdir += (5*look*sysvideo.height)/SCREENHEIGHT;
			if(player->lookdir > MAX_LOOKDIR || player->lookdir < MIN_LOOKDIR)
			{
				player->lookdir = cur_lookdir;
			}
		}
	}
	if(player->centering)
