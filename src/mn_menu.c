	...
	for (i = 0; i < 6; i++)
	{
		...
		else
		{
			...
			if (!home) {
				...
				sprintf(name, "%s/.heretic/"SAVEGAMENAME"%d.hsg", home,i);
			}
		}
		fp = fopen(name, "rb+");
		if (!fp)
...
static boolean SCLoadGame(int option)
{
	...
	else
	{
		...
		if (!home) {
			...
			sprintf(name, "%s/.heretic/"SAVEGAMENAME"%d.hsg", home,option);
		}
	}
	G_LoadGame(name);
	MN_DeactivateMenu();
...
void MN_DrawInfo(void)
{
	...
	I_SetPalette(W_CacheLumpName("PLAYPAL", PU_CACHE));
	src = (byte *)W_CacheLumpNum(W_GetNumForName("TITLE")+InfoType,PU_CACHE);
