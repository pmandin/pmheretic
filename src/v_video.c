...
#define TO_COORD_X(x)	(((x)*sysvideo.width)/SCREENWIDTH)
#define TO_COORD_Y(y)	(((y)*sysvideo.height)/SCREENHEIGHT)
...
void V_DrawPatch(int x, int y, patch_t *patch)
{
	...
	{
		I_Error("Bad V_DrawPatch: %d,%d",x,y);
	}
	col = 0;
	desttop = screen+TO_COORD_Y(y)*sysvideo.pitch+TO_COORD_X(x);
	w = TO_COORD_X(SHORT(patch->width));
	...
	for(; col < w; x++, col++, desttop++)
	{
		...
		while(column->topdelta != 0xff)
		{
			...
			dest = desttop+TO_COORD_Y(column->topdelta)*sysvideo.pitch;
			count = TO_COORD_Y(column->length);
...
void V_DrawPatchDirect(int x, int y, patch_t *patch)
{
	...
	if(x < 0 || x+SHORT(patch->width) > sysvideo.width || y < 0
		|| y+SHORT(patch->height) > sysvideo.height)
	{
		I_Error("Bad V_DrawPatch: %d,%d",x,y);
	}
	col = 0;
	desttop = screen+y*sysvideo.pitch+x;
	w = SHORT(patch->width);
	...
	for(; col < w; x++, col++, desttop++)
	{
		...
		{
			source = (byte *)column+3;
			dest = desttop+column->topdelta*sysvideo.pitch;
			count = column->length;
...
// step through the posts in a column
		...
		while (column->topdelta != 0xff )
		{
			...
			while (count--)
			{
				*dest = tinttable[((*dest)<<8) + source[frac>>16]];
				dest += sysvideo.pitch;
				frac += fracstep;
			}
			column = (column_t *)(  (byte *)column + column->length + 4 );
...
void V_DrawFuzzPatchDirect (int x, int y, patch_t *patch)
{
	int			count,col;
	column_t	*column;
	byte		*desttop, *dest, *source;
	...
	if (x<0||x+SHORT(patch->width) >sysvideo.width || y<0 || y+SHORT(patch->height)>sysvideo.height)
		I_Error ("Bad V_DrawPatch");
...
void V_DrawShadowedPatch(int x, int y, patch_t *patch)
{
	...
	col = 0;
	desttop = screen+TO_COORD_Y(y)*sysvideo.pitch+TO_COORD_X(x);
	desttop2 = screen+TO_COORD_Y(y+2)*sysvideo.pitch+TO_COORD_X(x+2);
...
// step through the posts in a column
		...
		while (column->topdelta != 0xff )
		{
			...
			source = (byte *)column + 3;
			dest = desttop + TO_COORD_Y(column->topdelta)*sysvideo.pitch;
			dest2 = desttop2 + TO_COORD_Y(column->topdelta)*sysvideo.pitch;
			count = TO_COORD_Y(column->length);
...
void V_DrawShadowedPatchDirect(int x, int y, patch_t *patch)
{
	int			count,col;
	column_t	*column;
	byte		*desttop, *dest, *source;
	...
	col = 0;
	desttop = screen+y*sysvideo.pitch+x;
	desttop2 = screen+(y+2)*sysvideo.pitch+x+2;
...
// step through the posts in a column
		...
		while (column->topdelta != 0xff )
		{
			source = (byte *)column + 3;
			dest = desttop + column->topdelta*sysvideo.pitch;
			dest2 = desttop2 + column->topdelta*sysvideo.pitch;
			count = column->length;
...
void V_DrawRawScreen(byte *raw)
{
	...
	for (y=0;y<sysvideo.height;y++) {
		int y1;
		byte *dest_line, *src_line;
	
		dest_line = dest;
		y1 = (y*SCREENHEIGHT)/sysvideo.height;
		src_line = raw+y1*SCREENWIDTH;
