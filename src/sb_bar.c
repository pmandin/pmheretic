...
static void ShadeLine(int x, int y, int height, int shade)
{
	...
	dest = screen+y*sysvideo.pitch+x;
	while(height--)
	{
		*(dest) = *(shades+*dest);
		dest += sysvideo.pitch;
...
static void ShadeChain(void)
{
	...
	for(i = 0; i < TO_COORD_X(16); i++)
	{
		ShadeLine(TO_COORD_X(277)+i, TO_COORD_Y(190), TO_COORD_Y(10), ((i*SCREENWIDTH)/sysvideo.width)/2);
		ShadeLine(TO_COORD_X(19)+i, TO_COORD_Y(190), TO_COORD_Y(10), 7-(((i*SCREENWIDTH)/sysvideo.width)/2));
	}
}
...
void SB_Drawer(void)
{
	...
	if(viewheight == sysvideo.height && !automapactive)
	{
		DrawFullScreenStuff();
		SB_state = -1;
	}
...
{
	DrINumber(fps, SCREENWIDTH-(SHORT(PatchINumbers[0]->width)+1)*3,0);
}
